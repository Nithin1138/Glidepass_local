...
const Navbar = ({ globalMouseX, globalMouseY }: { globalMouseX: any; globalMouseY: any }) => {
  ...
  const bookmarkletCode = `javascript:(function(){if(window.__lanpad_active){var e=document.getElementById("__gp_note");if(e){e.innerHTML="<span style='color:#f59e0b;font-size:18px;'>🛰️</span> ALREADY ACTIVE";e.style.opacity="1";e.style.transform="translate(-50%,0)";setTimeout(function(){e.style.opacity="0";e.style.transform="translate(-50%,-40px)";},3000);}return;}window.__lanpad_active=true;window.__gp_abort=false;try{var op=Event.prototype.preventDefault;Event.prototype.preventDefault=function(){if(this&&this.type&&["copy","paste","cut","beforeinput","selectstart","contextmenu","dragstart"].indexOf(this.type)!==-1){return;}if(op){return op.apply(this,arguments);}};}catch(err){}try{var og=window.getSelection;var fakeSel=function(){var sel=null;try{sel=og.call(window);}catch(e){}if(sel){sel.removeAllRanges=function(){};sel.empty=function(){};}return sel;};window.getSelection=fakeSel;document.getSelection=fakeSel;}catch(err){}function ul(r){if(!r)return;var ev=["copy","paste","cut","contextmenu","selectstart","dragstart"];ev.forEach(function(t){try{r.addEventListener(t,function(e){e.stopImmediatePropagation();},true);}catch(e){}});try{var al=r.querySelectorAll?r.querySelectorAll("*"):[];for(var i=0;i<al.length;i++){var el=al[i];if(el.shadowRoot)ul(el.shadowRoot);el.onselectstart=null;el.oncontextmenu=null;el.ondragstart=null;if(el.style){el.style.setProperty("user-select","text","important");el.style.setProperty("-webkit-user-select","text","important");}if(el.hasAttribute("unselectable"))el.removeAttribute("unselectable");}}catch(err){}}ul(document);try{var ob=new MutationObserver(function(){ul(document);});ob.observe(document.documentElement,{childList:true,subtree:true});}catch(err){}try{var s=document.createElement("style");s.innerHTML="*,*::before,*::after{-webkit-user-select:text!important;-moz-user-select:text!important;-ms-user-select:text!important;user-select:text!important;pointer-events:auto!important;}";document.head.appendChild(s);}catch(err){}try{var n=document.createElement("div");n.id="__gp_container";n.innerHTML="<div id='__gp_note' style='position:fixed;top:24px;left:50%;transform:translateX(-50%);background:rgba(5,5,5,0.85);color:#fff;padding:14px 24px;border-radius:16px;border:1px solid rgba(0,119,192,0.3);backdrop-filter:blur(12px);font-family:sans-serif;font-weight:900;font-size:14px;z-index:2147483647;display:flex;align-items:center;gap:10px;box-shadow:0 20px 40px rgba(0,119,192,0.2),inset 0 1px 0 rgba(255,255,255,0.1);transition:all 0.4s cubic-bezier(0.18,0.89,0.32,1.28);opacity:0;transform:translate(-50%,-40px);'><span style='color:#0077c0;font-size:18px;'>🛰️</span> LANPAD ACTIVATED</div>";document.body.appendChild(n);}catch(err){}function showN(t,c){c=c||"#0077c0";var e=document.getElementById("__gp_note");if(!e)return;e.innerHTML="<span style='color:"+c+";font-size:18px;'>🛰️</span> "+t;e.style.opacity="1";e.style.transform="translate(-50%,0)";setTimeout(function(){e.style.opacity="0";e.style.transform="translate(-50%,-40px)";},3000);}setTimeout(function(){showN("LANPAD ACTIVATED");},10);var lastId=localStorage.getItem("__gp_last_id")||"";var seenIds=new Set();var seenTxt=new Map();var queue=[];var wait=function(ms){return new Promise(function(res){setTimeout(res,ms);});};async function poller(){while(true){if(window.__gp_abort_poller)break;try{var res=await fetch("http://127.0.0.1:8000/api/v1/paste/poll?last_id="+lastId+"&t="+Date.now(),{headers:{"x-device-id":"b8b989d6-dca0-4d98-a0e4-2556c5fbc4a1"},cache:"no-store",mode:"cors",credentials:"omit"});var data=await res.json();if(data.status==="success"){lastId=data.id;localStorage.setItem("__gp_last_id",lastId);var txt=data.text||"";var mode=data.mode||"";if(mode==="system"||txt.indexOf("STOP_PASTE")!==-1){window.__gp_abort=true;queue=[];showN("PASTING STOPPED","#ef4444");continue;}var now=Date.now();var txtHash=txt.substring(0,100);if(seenIds.has(data.id)||(seenTxt.has(txtHash)&&now-seenTxt.get(txtHash)<2000))continue;seenIds.add(data.id);seenTxt.set(txtHash,now);queue.push(data);}else{await wait(500);}}catch(e){console.log("GP Poll Error:",e);await wait(2000);}}}async function executor(){while(true){if(window.__gp_abort){queue=[];window.__gp_abort=false;await wait(100);continue;}if(queue.length>0){var data=queue.shift();if(!data)continue;var wpm=data.wpm||40;var txt=data.text;var isRealistic=data.realistic||false;var el=document.activeElement;if(el&&("value"in el||el.isContentEditable)){var inject=function(c){if("value"in el){var start=el.selectionStart;var end=el.selectionEnd;if(start!==undefined&&start!==null){el.value=el.value.substring(0,start)+c+el.value.substring(end);el.selectionStart=el.selectionEnd=start+c.length;}else{el.value+=c;}}else{try{var sel=window.getSelection();if(sel&&sel.rangeCount){var range=sel.getRangeAt(0);range.deleteContents();range.insertNode(document.createTextNode(c));range.collapse(false);sel.removeAllRanges();sel.addRange(range);}}catch(e){}}try{el.dispatchEvent(new Event("input",{bubbles:true}));}catch(e){}try{el.dispatchEvent(new Event("change",{bubbles:true}));}catch(e){}};if(isRealistic){for(var i=0;i<txt.length;i++){if(window.__gp_abort)break;inject(txt[i]);var d=60000/(wpm*5);if(txt[i]===" ")d*=1.2;await wait(d*(0.8+Math.random()*0.4));}}else{if(!window.__gp_abort)inject(txt);}}}await wait(100);}}poller();executor();})();`;
    ...
    >
      ...
      <section id="downloads" className="min-h-screen flex flex-col justify-center py-24 relative overflow-hidden font-dmsans">
        ...
        <div 
          ...
          className="relative overflow-hidden w-full py-16 group/curtain bg-[#468FEA] border-t-8 border-white/30 min-h-[420px] flex flex-col items-center justify-center"
        >
