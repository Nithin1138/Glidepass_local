@echo off
REM ─────────────────────────────────────────────────────────────────────────────
REM LANpad Windows Native Messaging Host wrapper
REM
...
REM
REM This .bat is what ``com.lanpad.launcher.json`` points to.
REM It is equivalent in role to ``lanpad_bridge.sh`` on macOS / Linux.
REM ─────────────────────────────────────────────────────────────────────────────

setlocal

REM Resolve the directory this .bat lives in (i.e. the LANpad project)
set "DIR=%~dp0"
...
if %errorlevel% equ 0 (
    ...
    if %errorlevel% equ 0 (
        ...
        echo [LANpad] ERROR: Python is not installed or not in PATH. 1>&2
        exit /b 1
    )
)
