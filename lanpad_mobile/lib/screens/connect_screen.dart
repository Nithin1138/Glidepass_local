...
class _ConnectScreenState extends State<ConnectScreen> with TickerProviderStateMixin {
  ...
  Future<void> _connectManually() async {
    ...
    if (deviceName.isEmpty || sessionCode.isEmpty) {
      TopToast.show(context, 'Please enter both Device Name and Session Code', isError: true);
      return;
    }
    ...
    try {
              ...
              if (res.statusCode == 200) {
                ...
                if (data['status'] == 'success') {
                  ...
                  if (name.toLowerCase() == deviceName.toLowerCase() && foundUrl == null) {
                    foundUrl = url;
                    serverCode = data['session_code']?.toString();
      ...
      if (foundUrl != null) {
        if (serverCode != null && serverCode!.isNotEmpty && serverCode!.trim().toLowerCase() != sessionCode.toLowerCase()) {
          TopToast.show(context, 'Incorrect Session Code', isError: true);
          return;
        }
      ...
      setState(() => _isManualSearching = false);
      TopToast.show(context, 'Search failed: $e', isError: true);
    }
  }
